...
## Interactive map — forest loss (2021 → 2024)
...
disappeared between 2021 and 2024. Using a geometric `overlay()`, we isolate
only the areas that were forested in 2021 but are no longer in 2024 —
providing a spatially precise picture of deforestation hotspots.

::: {.callout-caution}
## Exercise 8 — Map forest loss (2021 → 2024) with a heatmap

**Goal:** Display the spatial distribution of forest loss across the NUTS3
region using a single `folium.plugins.HeatMap` layer. Forest polygons
(labels 2, 3, 4) are compared geometrically between 2021 and 2024 using
`.overlay()` to isolate areas that genuinely lost tree cover.
...
3. Use `.overlay(..., how="difference")` to compute forest lost between
   2021 and 2024
4. Recompute `area_km2` after the overlay and filter out micro-polygons
5. Reproject to EPSG:4326, compute centroids, build `heat_data` as
   `[lat, lon, area_km2]`
6. Create a `folium.Map` and add a single `HeatMap` layer with a green
   gradient
:::
...
# Forest present in 2021 but absent in 2024 = lost forest
...
forest_lost = forest_lost[forest_lost["area_km2"] > __]  # TODO: threshold (float)
...
HeatMap(
    ...
    gradient={0.4: "lightgreen", 0.7: "forestgreen", 1.0: "darkgreen"},
...
- `.dissolve()` merges all forest polygons of a year into one multipolygon,
  which makes the overlay much faster and avoids duplicate intersections.
- `forest_2021.overlay(forest_2024, how="difference")` returns the parts of
  2021 forest not covered by 2024 forest — i.e. areas that lost tree cover.
